// CUDAlings 12.01 — Naive matrix multiply
//
// C[M,N] = A[M,K] * B[K,N]. Each thread computes one element of C with a
// dot product across K.
//
// Goal: implement the naive kernel. Validate by checking the trace of
// A=ones(M,K), B=ones(K,N): every C[i,j] = K, trace = M*K when M==N.
