// CUDAlings 05.02 — Array of Structs vs Struct of Arrays
//
// AoS: struct P { float x, y, z; };  P points[N];
//   Reading points[t].x gives strided access (stride = sizeof(P) = 12B).
// SoA: float x[N], y[N], z[N];
//   Reading x[t] across the warp is coalesced (4B contiguous).
//
// Goal: given an AoS input `pts_aos`, produce three SoA outputs `xs/ys/zs`.
...
__global__ void to_soa(const P* aos, float* xs, float* ys, float* zs) {
    ...
    if (i < N) {
        // TODO: xs[i] = aos[i].x; ys[i] = aos[i].y; zs[i] = aos[i].z;
    }
}
