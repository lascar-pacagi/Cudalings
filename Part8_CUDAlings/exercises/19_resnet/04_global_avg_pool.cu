// CUDAlings 19.04 — Global Average Pool kernel
//
// y[b, c] = mean over (h, w) of x[b, c, h, w]    -- (B, C, H, W) -> (B, C)
//
// This is the layer that replaces the giant fully-connected head in a
// ResNet, dropping the parameter count from millions to ~num_classes * C.
//
// Goal: one block per (b, c); threads cooperate to sum H*W elements.
...
int main() {
    float h_x[B*C*H*W];
    for (int i = 0; i < B*C*H*W; ++i) h_x[i] = 1.0f;     // mean = 1
    float h_y[B*C] = {0};
    float *d_x, *d_y;
