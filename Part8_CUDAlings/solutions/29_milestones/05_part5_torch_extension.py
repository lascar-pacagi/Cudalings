...
if not torch.cuda.is_available():
    ...
    mod = load_inline(name="cudalings_milestone5", cpp_sources=[CPP_SRC],
                      cuda_sources=[CUDA_SRC], functions=["fwd", "bwd"], verbose=False)
    class FusedReluScale(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x, alpha):
            ctx.save_for_backward(x)
            ctx.alpha = alpha
            return mod.fwd(x, alpha)
        @staticmethod
        def backward(ctx, dy):
            (x,) = ctx.saved_tensors
            return mod.bwd(x, dy, ctx.alpha), None
    ...
    print("ok" if (fwd_ok and bwd_ok) else "FAIL")
